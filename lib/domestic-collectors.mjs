...
import { parseFeed, request, eachTarget, readWatchlist } from "./acquisition.mjs";
...
export async function serviceJson(url, options = {}, fetcher = fetch) {
  const loginRoute = /\/login\/(status|qrcode)/.test(url);
  const searchRoute = new URL(url).pathname === "/api/v1/feeds/search";
  const timeoutMs = loginRoute || searchRoute ? 90000 : 30000;
  let response;
  try {
    response = await fetcher(url, { ...options, redirect: "error", signal: AbortSignal.timeout(timeoutMs) });
  } catch (error) {
    if (error.name === "TimeoutError" || error.name === "AbortError") {
      throw failure("timeout", `${searchRoute ? "小红书搜索" : loginRoute ? "小红书登录检查" : "采集请求"}超过 ${timeoutMs / 1000} 秒未完成，请稍后重试；这不代表登录已失效。`);
    }
...
function candidate(sourceId, record, mode) {
  ...
  const text = plainText(record.text || record.html || "").slice(0, 50000);
  return {
    id, type: "hotnews", name: plainText(title, 300) || "待研究线索", tagline: plainText(text, 240) || "自动采集的内容线索，项目身份待核实。",
    country: "未知", stage: "热点资讯 / 待关联项目", status: "new", sampleMode: false,
...
export async function collectXiaohongshu(config, limit, fetcher = fetch, collectOptions = parseCollectOptions({ limit })) {
  const local = await xhsLoginState();
  if (!local.loggedIn) throw failure("needs_login", `${local.reason}。请点击「扫码登录」用 App 扫码后再采集。`);
  ...
  if (login.data?.is_logged_in !== true) throw failure("needs_login", "小红书尚未登录，请点击「扫码登录」用 App 扫码。");
  const items = [], warnings = [], seen = new Set();
  const searchKeywords = config.xhsKeywords?.length ? config.xhsKeywords : config.keywords;
  if (!searchKeywords?.length) throw failure("needs_setup", "请配置小红书搜索关键词（如 ai startup、ai github）。");
  for (const keyword of searchKeywords) {
    ...
    try {
      search = await request("/api/v1/feeds/search", {
        ...
        body: JSON.stringify({
          keyword,
          filters: { sort_by: "最新", publish_time: collectOptions.xhsPublishTime || "一周内" },
        }),
      });
      if (!Array.isArray(search.data?.feeds)) throw failure("invalid_response", "小红书搜索响应缺少 feeds。");
    } catch (error) {
      warnings.push({ code: error.code || "upstream_error", message: `搜索「${keyword}」失败：${error.message}` });
      continue;
    }
  ...
  return { items: filterByTimeRange(items, collectOptions), warnings };
}

export async function collectDomestic(sourceId, config, limit = 12, fetcher = fetch, collectOptions = parseCollectOptions({ limit })) {
  if (sourceId === "zhihu" && !config.routes.zhihu?.length) return collectZhihuHot(limit, fetcher, config.zhihuKeywords);
  if (sourceId === "douyin" && process.env.DOUYIN_API_BASE_URL) {
    const targets = (await readWatchlist()).douyin;
    if (!targets?.length) throw failure("needs_target", "请在 data/watchlist.local.json 添加抖音作者主页 sec_user_id；服务端可能需要本人 Cookie");
    return eachTarget(targets, async target => {
      if (!target.secUserId) throw failure("needs_target", "缺少抖音作者 sec_user_id");
      const url = new URL(`${process.env.DOUYIN_API_BASE_URL.replace(/\/$/, "")}/api/douyin/web/fetch_user_post_videos`);
      url.searchParams.set("sec_user_id", target.secUserId); url.searchParams.set("max_cursor", "0"); url.searchParams.set("count", String(Math.min(35, limit)));
      const response = await request(url.href, {}, fetcher);
      const data = await response.json();
      if (data.code !== undefined && data.code !== 200) throw failure("upstream_error", "抖音服务返回错误，请检查登录态或验证码");
      const payload = data.data;
      if (payload?.status_code !== undefined && payload.status_code !== 0) throw failure("needs_login", "抖音拒绝请求，请在服务端更新本人的 Cookie");
      const parsed = parseSocialExport(JSON.stringify(payload), "douyin");
      if (parsed.errors.length) throw failure("invalid_response", "抖音响应缺少视频链接");
      return { items: parsed.items.map(i => ({ ...i, collection: { ...i.collection, mode: "douyin_api", targetId: target.id, targetName: target.name } })), hasMore: Boolean(payload.has_more) };
    });
  }
  ...
  if (sourceId === "xiaohongshu") {
    const batch = await collectXiaohongshu(config, limit, fetcher, collectOptions);
    const keywords = config.xhsKeywords?.length ? config.xhsKeywords : DEFAULT_INTENT_KEYWORDS;
    return { ...batch, items: applyIntentBatch(batch.items, keywords) };
  }
  ...
  for (const route of routes) {
    ...
    try {
      ...
      const normalized = normalizeJsonFeed(feed, sourceId, sourceId === "zhihu" ? config.zhihuKeywords : config.keywords);
  ...
  const keywords = sourceId === "zhihu"
    ? (config.zhihuKeywords?.length ? config.zhihuKeywords : DEFAULT_INTENT_KEYWORDS)
    : (config.keywords?.length ? config.keywords : DEFAULT_INTENT_KEYWORDS);
  return { items: applyIntentBatch(filterByTimeRange(items, collectOptions), keywords), warnings };
}
