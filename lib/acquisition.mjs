...
import { filterEmergingTargets, isIncumbentGithubOrg, isStaleStartup } from "./discovery.mjs";
import { applyIntentBatch, DEFAULT_INTENT_KEYWORDS } from "./intent-discovery.mjs";

export const acquisitionSourceIds = ["yc", "jike", "x", "feeds", "github_kol", "hf_spaces", "producthunt", "reddit"];
const DEFAULT_YC_TARGETS = [
  { id: "ai", name: "YC AI", path: "tags/artificial-intelligence.json", enabled: true },
];
...
export function rawItem(sourceId, record, target, mode) {
  ...
  return {
    ...
    tagline: plainText(text, sourceId === "x" ? 2000 : 240), country: "未知", stage: "原始内容 / 未分析", status: "new", sampleMode: false,
...
async function xEmbedTimeline(target, limit, fetcher) {
...
async function xApiTimeline(target, limit, fetcher) {
  if (!process.env.X_BEARER_TOKEN) throw fail("needs_key", "配置 X_BEARER_TOKEN 后使用官方 API；无 Token 时可设 X_MODE=embed");
  ...
  return { items: items.filter(Boolean), hasMore: Boolean(nextToken), coverage: "x_api_v2_timeline" };
}

async function xTimeline(target, limit, fetcher) {
  // 有 Bearer Token 时始终走官方 API，不回退 embed。
  if (process.env.X_BEARER_TOKEN) {
    return xApiTimeline(target, limit, fetcher);
  }
  if (process.env.X_MODE === "embed") {
    return xEmbedTimeline(target, limit, fetcher);
  }
  throw fail("needs_key", "在本机 .env 配置 X_BEARER_TOKEN 并开通读取额度，或设置 X_MODE=embed 使用公开嵌入窗口");
}
...
export async function collectAcquisition(sourceId, limit = 12, fetcher = fetch, watchlist, collectOptions = parseCollectOptions({ limit }), intentKeywords = DEFAULT_INTENT_KEYWORDS) {
  ...
  const rawTargets = config[sourceId]?.length ? config[sourceId] : (sourceId === "yc" ? DEFAULT_YC_TARGETS : []);
  const targets = filterEmergingTargets(rawTargets);
  const fetchLimit = Math.min(100, limit * 3);
  const result = await eachTarget(targets, async target => {
    if (sourceId === "yc") {
      const path = String(target.path || target.value || "tags/artificial-intelligence.json").replace(/^\//, "");
      const response = await request(`${YC_API_BASE}/${path}`, {}, fetcher);
      ...
      const items = kept.map((r) => rawItem("yc", {
        id: r.id || r.slug, title: r.name, text: r.long_description || r.one_liner,
        url: r.url || r.website, links: [r.website], raw: r, coverage: "active_recent_batches",
      }, target, "yc_oss_api")).filter(Boolean);
      return { items, coverage: "active_recent_batches" };
    }
    if (sourceId === "jike") {
    ...
    if (sourceId === "x") {
      ...
      const paidApi = timeline.coverage === "x_api_v2_timeline";
      const items = paidApi
        ? timeline.items.slice(0, Math.max(limit, timeline.items.length))
        : filterByTimeRange(timeline.items, options).slice(0, limit);
    ...
    if (sourceId === "hf_spaces") {
      const params = new URLSearchParams({ sort: "createdAt", direction: "-1", limit: String(Math.min(fetchLimit, 100)) });
      ...
      const data = await json(`https://huggingface.co/api/spaces?${params}`, { headers }, fetcher);
  ...
  // YC directory already filters by batch/status; do not drop by UI time window.
  const timed = sourceId === "yc" ? result.items : filterByTimeRange(result.items, options);
  return { ...result, items: applyIntentBatch(timed, keywords) };
}
