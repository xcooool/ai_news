...
async function xTimeline(target, limit, fetcher) {
  const mode = process.env.X_MODE || (process.env.X_BEARER_TOKEN ? "api" : "embed");
  if (mode === "embed") {
    return xEmbedTimeline(target, limit, fetcher);
  }
  if (!process.env.X_BEARER_TOKEN) throw fail("needs_key", "在本机 .env 配置 X_BEARER_TOKEN 并开通读取额度，或设置 X_MODE=embed 使用公开嵌入窗口");
  const headers = { Authorization: `Bearer ${process.env.X_BEARER_TOKEN}` };
  let lookup;
  try {
    lookup = await json(`https://api.x.com/2/users/by/username/${encodeURIComponent(target.handle)}?user.fields=public_metrics,description`, { headers }, fetcher);
  } catch (error) {
    if (error.code === "needs_credits" || String(error.message || "").includes("HTTP 402")) {
      return xEmbedTimeline(target, limit, fetcher);
    }
    throw error;
  }
