...
export async function runCollectors({ sources = ["github", "hackernews", "huggingface"], limit = 12, connectors, fetcher } = {}) {
  const startedAt = new Date().toISOString();
  const results = [];
  const errors = [];
  const sourceResults = [];
  const config = connectors ?? (sources.some(id => domesticSourceIds.includes(id)) ? await readConnectors() : null);

  for (const source of sources) {
    const before = results.length;
    try {
      if (domesticSourceIds.includes(source)) {
        const batch = await collectDomestic(source, config, limit, fetcher);
        results.push(...batch.items);
        errors.push(...batch.warnings.map(warning => ({ sourceId: source, ...warning, at: new Date().toISOString() })));
        sourceResults.push({ sourceId: source, itemCount: batch.items.length,
          status: batch.warnings.length ? (batch.items.length ? "partial" : "error") : "ok" });
        continue;
      }
