...
export function defaultConnectors() {
  return {
    ...
    keywords: ["ai github", "ai startup", "ai startup hiring", "ai hot github"],
    xhsKeywords: ["ai github", "ai startup", "ai startup hiring", "ai hot github"],
    zhihuKeywords: ["AI", "人工智能", "智能体", "大模型", "开源", "机器学习", "ChatGPT", "DeepSeek"],
    wechatFeedIds: ["all"],
...
export function catalogWithConnections(catalog, config, runs = []) {
  return catalog.map(source => {
    const configured = source.id === "jike" ? "implemented" : domesticSourceIds.includes(source.id) ? connectorReadiness(source.id, config) : source.status;
    ...
    const setup = domesticSourceIds.includes(source.id) || source.id === "feeds" ? describeSetup(source.id, config) : null;
